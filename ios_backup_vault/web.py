...
async function show(v){const m=document.getElementById('main');m.textContent='로딩…';try{
 ...
 else if(v=='appscan'){const a=await j('/api/appscan');m.innerHTML='<h2>백업 앱 스캔</h2>'+(a.length?('<table><tr><th>앱</th><th>파일수</th><th>내용 읽기</th><th>비고</th></tr>'+a.map(x=>`<tr><td>${esc(x.label)}</td><td>${x.file_count}</td><td>${x.readable?'✅ 가능':'❌ 불가'}</td><td>${esc(x.note)}</td></tr>`).join('')+'</table>'):'<p>알려진 메신저 앱이 백업에 없습니다.</p>')}
 }catch(e){err(m,e)}}
